...
half3 SampleReflections(float eyeIndex, half3 normalWS, half3 viewDirectionWS, half2 screenUV,
    half fresnelTerm, half roughness)
{
    half3 reflection = 0;
    half2 refOffset = 0;
...
//#elif _REFLECTION_PLANARREFLECTION
    ...
    half2 reflectionUV = screenUV + normalWS.zx * half2(0.05, 0.1) * _WaveRefraction;
    half2 reflectionUV2 = screenUV - normalWS.xz * half2(0.1, 0.05) * _WaveRefraction;
    reflectionUV = (reflectionUV+reflectionUV2) / 2;
    //half2 reflectionUV = screenUV;

    if(eyeIndex == 0.){
        reflection += SAMPLE_TEXTURE2D(_ReflectionTex, sampler_ReflectionTex, reflectionUV).rgb;//planar reflection
        //reflection += SAMPLE_TEXTURE2D_LOD(_ReflectionTex, sampler_ReflectionTex, reflectionUV, 6 * roughness).rgb;//planar reflection
        //return SAMPLE_TEXTURE2D (_ReflectionTex, sampler_ReflectionTex, uv);
    }
    else{
        reflection += SAMPLE_TEXTURE2D(_ReflectionTexOther, sampler_ReflectionTexOther, reflectionUV).rgb;//planar reflection
        //reflection += SAMPLE_TEXTURE2D_LOD(_ReflectionTexOther, sampler_ReflectionTexOther, reflectionUV, 6 * roughness).rgb;//planar reflection
...
//#endif
    ...
    return reflection *fresnelTerm* _ReflectionIntensity;
