...
namespace AkilliMum.SRP.Mirror
{
    ...
    /// </summary>
    public class GerstnerWavesJobsInstance
    {
        //General variables
        ...
        private NativeArray<Wave> _waveData; // Wave data from the water system
        ...
        readonly Dictionary<int, int2> Registry = new Dictionary<int, int2>();

        public void Init(CameraShadeWater water)
        {
            //Wave data
            //_waveCount = water._waves.Length;
            _waveCount = water.GetWaves().Length;
            _waveData = new NativeArray<Wave>(_waveCount, Allocator.Persistent);
            for (var i = 0; i < _waveData.Length; i++)
            {
                //_waveData[i] = water._waves[i];
                _waveData[i] = water.GetWaves()[i];
            }
        ...
        public void UpdateHeights()
        {
            ...
            // Buoyant Object Job
            var waterHeight = new GerstnerWavesJobsInstance.HeightJob()
            {
                WaveData = _waveData,
                Position = _positions,
