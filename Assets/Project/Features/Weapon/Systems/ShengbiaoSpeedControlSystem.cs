...
namespace Project.Features.Weapon.Systems {
    ...
    public sealed class ShengbiaoSpeedControlSystem : ISystemFilter {
        ...
        {
            if (entity.Read<ReloadTimeDefault>().Value - entity.Read<ReloadTime>().Value < GameConsts.Weapons.Shengbiao.ATTACK_SECONDS) return;

            var backwardsRatio =  (GameConsts.Weapons.Shengbiao.MIN_LENGHT - entity.Read<ShengbiaoWeapon>().Offset) / (entity.Read<ReloadTime>().Value);
            entity.Get<ShengbiaoWeapon>().MoveRatio = -backwardsRatio;
            entity.Read<ShengbiaoDamageSpot>().Value.Get<ProjectileParent>().Speed = GameConsts.Weapons.Shengbiao.RETURN_SPEED;
